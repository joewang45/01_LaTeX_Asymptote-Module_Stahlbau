 ...
 picture crosshatch(real H=5mm, pen p=currentpen)
{
 ...
 picture beton(real H=5mm, pen p=currentpen){
  ...
  real c=cos(theta);
    ...
    draw(tiling,shift(-h/4,y/4)*g,p+dashed);
    draw(tiling,shift(h/4,-y/4)*g,p+dashed);
 ...
 picture estrich(real H=5mm, pen p=currentpen){
 ...
 picture color(pen p=currentpen){
